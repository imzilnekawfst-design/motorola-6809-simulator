...
//Un panel est un conteneur graphique qui sert à regrouper des composants (boutons, zones de texte, etc.).Il permet d’organiser l’interface graphique.
//Le ContentPane est le conteneur principal de la fenêtre.Tous les composants visibles (boutons, zones de texte, panels) y sont ajoutés.C’est la surface principale de travail de la fenêtre.
//Un ScrollPanel est un composant qui ajoute des barres de défilement verticales et/ou horizontales.
public class Assembleur extends JFrame {
	...
	//Stocke les étiquettes (labels) du programme assembleur.
	...
	public Assembleur() {
		...
		setAlwaysOnTop(true);
		setResizable(false);
		//Cache la fenêtre sans fermer le programme.

		setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
		//Définit la position et la taille de la fenêtre.
		setBounds(1000, 120, 250, 300);
		contentPane = new JPanel();//Crée le conteneur principal de la fenêtre.
		contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));//Ajoute une marge interne.
           contentPane.setBackground(Color.PINK );//Définit la couleur de fond.
		setContentPane(contentPane);//Applique le panel comme conteneur principal.
		contentPane.setLayout(null);//Utilise un placement manuel des composants.
		//Positionne et dimensionne la zone de texte.
		textArea.setBounds(10, 60, 220, 200);
		contentPane.add(textArea);//Ajoute la zone de texte au panel.
		
		
		
		JScrollPane scrollPane = new JScrollPane(textArea);//Ajoute des barres de défilement à la zone de texte.
        scrollPane.setBounds(10, 60, 220, 200); //Définit sa position et sa taille.
        contentPane.add(scrollPane);//Ajoute le scroll au panel.
        
        bouton_Reset.addActionListener(new ActionListener() { // Button pour effacer tout 
			public void actionPerformed(ActionEvent click) { //Efface tout, réinitialise l’exécution et réactive les boutons.
				ved_all();
				bouton_Execute.setEnabled(true);
				bouton_PasAPas.setEnabled(true);	
            }
		});
		//Positionne le bouton Reset.
		bouton_Reset.setBounds(10, 5, 220, 23);
		contentPane.add(bouton_Reset); //Ajoute le bouton au panel.
		
		bouton_PasAPas.addActionListener(new ActionListener() { // Button  pas a pas
			public void actionPerformed(ActionEvent click) {
				String Asmbl = textArea.getText(); //Récupère le code assembleur écrit.
				if( !etq_pretes ) {
					 trouver_Toutes_Les_Etiquettes(Asmbl); //Analyse le programme pour détecter les étiquettes.
...
private void sub_direct(char registre, String operande) {
	...
	//Lecture de la valeur stockée en RAM à cette adresse
...
private void sub_etendu(char registre, String operande) {
	...
	//Lecture de la mémoire à l’adresse 16 bits
    String valeur = (String) RAM.obtenirValeur(adresse);
